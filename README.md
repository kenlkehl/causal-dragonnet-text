...
`selected_feature_sets.json`, and `outer_cv_metrics.csv`. Parsimony review is
disabled by default; disabled runs record `skipped` and retain the reviewed
feature set. When enabled, the parsimony review runs before final forest fitting
and may record `retain_all` when ablations do not justify pruning. The same run
also writes skill-compatible aliases:

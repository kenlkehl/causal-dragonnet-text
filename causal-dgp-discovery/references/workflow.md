...
4. Fit HTR nuisance and R-stage/effect models. Add HTR nuisance predictions to the ensemble treatment/outcome predictions used for R-loss and pseudo-target construction, and expose highly attended tokens/spans to Codex for feature proposal.
5. Convert recurring text/span evidence into `ExplicitFeatureSpec`-shaped concepts with roles, categories, aliases, `value_aliases`, temporal windows, and missingness rules. Merge prespecified and Codex-proposed specs by name and role rather than extracting duplicates.

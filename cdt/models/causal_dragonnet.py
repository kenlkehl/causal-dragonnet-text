...
class CausalDragonnetText(nn.Module):
    ...
    - Binary treatment causal inference (DragonNet)
    """
    ...
    def __init__(
        ...
        # Binary treatment DragonNet - registered as submodule
    ...
    def forward(
        ...
        y0_logit, y1_logit, t_logit, phi = self.dragonnet(x_representation)
    ...
    def get_representation(
        ...
        with torch.no_grad():
            x_representation = self.feature_extractor(chunk_embeddings_list)
            _, _, _, phi = self.dragonnet(x_representation)
            return phi

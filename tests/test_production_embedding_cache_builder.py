...
class _FakeEncoder:
    def __init__(self, *, dimension: int = 5, max_seq_length: int = 64) -> None:
...
def _install_fake_encoder(monkeypatch, encoder=None, *, callback=None):
    ...
    def load(*, model_path, device, max_seq_length):
        observed.update(
            {
...
@pytest.mark.parametrize("selection", (None, "first"))
def test_requires_explicit_last_chunk_selection(
    tmp_path: Path,
    monkeypatch,
    selection: str | None,
):

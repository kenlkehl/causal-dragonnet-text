...
information**. They exist so that a completed method can be evaluated. They must
...
5. Use honest, observable validation data to find extraction failures, missing
   ...
   a bounded loop.
6. Combine the extracted variables with honest numerical outputs from the text
   models and fit a causal forest.
...
The final estimator must be a causal forest. If a causal forest cannot be fit, the
run must fail visibly. It must not silently substitute an S-learner or another
non-forest estimator.
...
the population, treatment coding, endpoint, follow-up window, and censoring rule
must be specified explicitly.

The repository calls its output an individual treatment effect, or ITE. More
...
column `true_ite_prob` records the data-generating difference between the two
potential binary-outcome probabilities for that row. That probability-scale target
...
construction. The benchmark metadata did not enforce positivity mechanically, so
overlap diagnostics remain necessary. The agents are not asked to police these
assumptions inside their feature-discovery or extraction prompts.
...
The agent is discovering variables. It is not estimating a treatment effect, and
it should not begin by forcing every clue into causal-role terminology. First name
and ground the feature. Decide how the feature might be used only afterward.
...
even when the explicit extraction step is imperfect.
...
specific variable, not merely a token, topic number, latent vector, or model score.

### Nuisance models and residual loss

In this project, a **nuisance model** is a supporting model of treatment assignment
or expected outcome. Those predictions are used to remove the part of treatment and
...
Roles are assigned only after a feature has been named and grounded:

- A **confounder** is a plausible cause of both treatment choice and outcome.
  Treatment prediction alone is insufficient. Evidence connecting the same feature
...
- An **effect modifier** may change the relative effect of the treatments.
...
Roles may overlap. For example, a feature can be both prognostic and an effect
...
evaluated on the union of the held-out predictions, one prediction per patient.
...
| `sparse_query_moments` | Inactive fallback | Fixed sparse-query activation | Not permitted as neural-query evidence in this benchmark | None in the required benchmark path |
...
A query moment by itself is non-grounding: its strength, sign, recurrence, or
stability cannot name a patient characteristic. A neural-query record may support a
feature only through its attached semantic witness or lexical foreground/background
contrast. The moment may prioritize or strengthen that witness. A citation is valid
only if it resolves to the witness-bearing record, not to an anonymous moment alone.
...
benchmark and must not be allowed to masquerade as learned neural-query evidence.
...
The causal forest may receive the corresponding row-level values directly. This
...
Every prompt must be understandable to an open-weight model that knows nothing
about this repository.

Use a short, stable system message and a concise pass-specific user message. Keep
JSON as the evidence container and output format, but explain the scientific task
before showing JSON.

Prompt rules:

- State the mission in plain language.
...
The main worktree is intentionally dirty and must not be reset. At this handoff it
contains modified runner/CLI files and tests, plus untracked remote utility scripts
and this brief.

The uncommitted main-tree changes include:
...
- Remote selector inference is available on Camus at port 8010, then 8020, then
  8030.

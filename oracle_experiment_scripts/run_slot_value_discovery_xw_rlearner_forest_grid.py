#!/usr/bin/env python
"""Run slot-value discovery X/W R-learner representation experiments."""
...
from run_oracle_experiments import _resolve_parquet_file
...
def _close_cache(cache: ConceptEmbeddingCache) -> None:
...
def _make_config(
    ...
    return XWRLearnerForestConfig(
        ...
        rlearner_nuisance_folds=(
            args.rlearner_nuisance_folds
            if args.rlearner_nuisance_folds is not None
            else args.n_folds
        ),
...
def _aggregate(output_dir: Path, results: List[Dict[str, Any]]) -> None:
    ...
    group_cols = [
        ...
        "svx_slot_dim",
        "svx_anchor_weight",
        "rlearner_effect_batch_size",
        "rlearner_effect_accumulation_steps",
        "learning_rate",
        "epochs",
    ]
    ...
    metric_cols = [
        col
        for col in ["ite_corr", "ite_spearman_corr", "ate_bias", "ite_mse", "ite_mae"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Slot-value discovery X/W R-learner -> CausalForestDML grid"
    )
    parser.add_argument("--datasets", nargs="+", default=DEFAULT_DATASETS)
    parser.add_argument(
        "--output-dir",
        "-o",
        default="../pcori_experiments/slot_value_xw_large_tau_batch",
    )
    ...
    parser.add_argument("--chunk-size-words", type=int, nargs="+", default=[96])
    ...
    parser.add_argument("--rlearner-effect-batch-size", type=int, default=64)
    parser.add_argument("--rlearner-effect-accumulation-steps", type=int, default=2)
...
def main() -> None:
    ...
    logger.info("Prepared %d slot_value_discovery X/W configs", len(configs))
    if args.dry_run:
        for config in configs:
    ...
    if pending_configs:
        ...
        logger.info(
            "Effect physical batch=%s, accumulation=%s, effective batch=%s",
            args.rlearner_effect_batch_size,
            args.rlearner_effect_accumulation_steps,
            args.rlearner_effect_batch_size * args.rlearner_effect_accumulation_steps,
        )

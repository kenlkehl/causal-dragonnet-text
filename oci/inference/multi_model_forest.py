...
class MultiModelForestRunner:
    ...
    def _build_handoff(self) -> None:
        if self.handoff_path.exists() and not self.force_stage1:
            ...
            return
        self.handoff_dir.mkdir(parents=True, exist_ok=True)
        # The primary numerical fit writes full-outer evidence for its own
        # diagnostics.  Stage 2 additionally needs every exact canonical inner
        # context, so build the complete handoff with the same context producer
        # used by the lightweight researcher workflow.  Never clone outer-fold
        # evidence into an inner scope.
